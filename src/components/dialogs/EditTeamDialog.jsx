...
export function EditTeamDialog({ teamDialogOpen, handleModalClose ,setSnackbarOpen, setSnackbarMessage}) {
  const [users, setUsers] = React.useState([]); // État pour stocker la liste des utilisateurs
  const [selectedUsers, setSelectedUsers] = useState([]);
  const [selectedNewResponsible, setSelectedNewResponsible] = useState(null); // État pour stocker le nouvel utilisateur responsable sélectionné

  const handleAutocompleteChange = (event, newValue) => {
    setSelectedUsers(newValue);
  };

  const handleNewResponsibleChange = (event, newValue) => {
    setSelectedNewResponsible(newValue);
  };
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        // Filtrer les utilisateurs en utilisant une seule condition complexe
      const teamId = localStorage.getItem("team");
      const filteredStudents = usersData.filter(
        (user) =>
          user.email !== localStorage.getItem("email") &&
          (!user.teamId || user.teamId === parseInt(teamId)) && 
          user.authorities.some(
            (authority) => authority.authority === "ROLE_STUDENT"
          )
      );
      // Mettre à jour l'état avec la liste filtrée d'étudiants
      setUsers(filteredStudents);
      // Ajouter un log pour suivre les utilisateurs récupérés
      console.log("Utilisateurs récupérés avec succès :", filteredStudents);
    } catch (error) {
  ...
  const handleSubmit =  (event) => {
    ...
    const teamData = {
      ...
      newResponsible: selectedNewResponsible.id ,
    };
    try {
      const token = localStorage.getItem("token");
      if (!token) {
      ...
      updateTeam(teamData,token, setSnackbarOpen, setSnackbarMessage);
       handleModalClose();
    } catch (error) {
    ...
    >
      <DialogTitle>
        <div style={{display:"flex",alignItems:"center" ,justifyContent:"space-between"}}>
          <Typography variant="h6">Update Team</Typography>
          <CloseIcon
      ...
      <DialogContent>
        <DialogContentText id="alert-dialog-slide-description">
          Please fill in the form to update your team
        </DialogContentText>
  ...
  <Autocomplete
    ...
    onChange={handleNewResponsibleChange}
